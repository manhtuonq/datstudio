...
const getStorageKey = (slug: string) => `datphoto_client_${slug}`;

const downloadPhoto = async (url: string, filename: string) => {
  try {
    // Use edge function proxy to bypass CORS and force download
    const { data: { session } } = await supabase.auth.getSession();
    const supabaseUrl = import.meta.env.VITE_SUPABASE_URL;
    const response = await fetch(`${supabaseUrl}/functions/v1/download-photo`, {
      method: "POST",
      headers: {
...
const ClientAlbumView = () => {
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    setClientId(data.id);
    setFormSubmitted(true);
    // Save to localStorage so they don't have to fill again
    if (slug) {
      localStorage.setItem(getStorageKey(slug), JSON.stringify({ clientId: data.id }));
    }
  };

  const openLightbox = useCallback(async (photo: Tables<"photos">) => {
                ...
                <div className="absolute inset-0 bg-foreground/0 group-hover:bg-foreground/20 transition-colors rounded-xl flex items-end justify-center pb-4 opacity-0 group-hover:opacity-100">
                  <div className="flex gap-2">
                    ...
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        downloadPhoto(photo.url, `photo-${idx + 1}.jpg`);
                      }}
        ...
        <div className="fixed inset-0 z-50 bg-foreground/90 flex items-center justify-center px-4" onClick={() => setLightboxPhoto(null)}>
          <div className="max-w-4xl w-full max-h-[90vh] flex flex-col md:flex-row gap-4" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="w-full md:w-80 bg-card rounded-xl p-4 overflow-y-auto max-h-[80vh]">
              ...
              {/* Download */}
              <button
                onClick={() => downloadPhoto(lightboxPhoto.url, `photo.jpg`)}
